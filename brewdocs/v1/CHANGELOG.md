...
- Flipped the `public.__lottery_game_fk_diag` view to `security_invoker` so the diagnostic helper respects caller RLS instead of triggering the Supabase `security_definer_view` lint.
- Aligned the current-tier display to `user_entitlements` only, keeping the legacy `tier` alias for compatibility but removing the stale auth-metadata fallback that let Pricing and Strategy Locker disagree for `command@brewlotto.app`.
