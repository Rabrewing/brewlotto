...
- BrewU now also has a dedicated Test Lab flow for approved QA testers. Family testers can sign in with approved tester emails, walk the app by tier, and submit yes/no + freeform QA reports with screenshots into a separate BrewCommand QA queue backed by `qa_reports` and `qa-screenshots`.
