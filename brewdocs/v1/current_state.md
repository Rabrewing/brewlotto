...
- TimePulse now lives on Pro via `timepulse_access`, while Master gets TimePulse II via `timing_analysis_access`. The billing and strategy locker surfaces read both flags so the tier ladder reads as a real progression instead of a renamed duplicate.
