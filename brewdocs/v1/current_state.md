...
- TimePulse now lives on Pro via the active Stripe-backed tier, while Master gets TimePulse II via `timing_analysis_access`. Billing and Strategy Locker now derive TimePulse access from the subscription tier so a lagging entitlement row does not drop BrewMaster users back to Free Explorer.
...
- The shared `useUserTier` hook still returns a legacy `tier` alias for compatibility, but the active current-tier display now comes from `user_entitlements` only so Pricing, Billing, and Strategy Locker cannot disagree about whether `command@brewlotto.app` is actually on Master.
...
- The `public.__lottery_game_fk_diag` helper view has been flipped to `security_invoker` so it respects caller RLS and stops tripping the Supabase security-definer-view lint.
- The helper-function search_path warnings and permissive legacy write-policy warnings have been cleaned up in a forward migration: `set_updated_at`, `tier_rank`, `next_scheduled_draw_at`, and `refresh_draw_freshness_status` now pin `search_path = public`, and the draw / `pick_history` write policies are service-role only.
- The anonymous GraphQL exposure pass now revokes anon access across the public schema and removes authenticated access from admin/internal tables, views, and SECURITY DEFINER RPCs; the remaining authenticated exposure is limited to the user-owned surfaces the app reads directly.
- The RLS-no-policy tables reported by Supabase now carry explicit `service_role` policies so the schema states that those admin/legacy tables are intentionally service-managed instead of policyless.
...
- Login is temporarily locked to BrewCommand-approved admin and QA tester allowlist accounts; the admin allowlist includes `command@brewlotto.app` plus the other internal addresses, and the QA tester list is seeded separately. Remove or widen that gate before public V1 launch if the full product needs broader access.
- The login form now optionally includes Cloudflare Turnstile for magic-link abuse protection, and the app only needs the public `NEXT_PUBLIC_TURNSTILE_SITE_KEY` plus the matching Supabase Auth secret when that layer is enabled.
...
- The Support category picker now uses a branded custom dropdown instead of the browser-native white menu so the support form stays visually consistent with the rest of BrewLotto.
- BrewU now also has a dedicated Test Lab flow for approved QA testers. Family testers can sign in with approved tester emails, see a gated intro overlay first, then walk the app by tier and submit yes/no + freeform QA reports with screenshots into a separate BrewCommand QA queue backed by `qa_reports` and `qa-screenshots`. Tester identity is auto-captured from the signed-in account so the form can stay simple, drafts are saved per tester email so they can stop and resume later without starting over, the initial tester roster is seeded in the shared auth helper and can still be extended via `BREWQA_TESTER_EMAILS` including `rb.brewington@gmail.com`, `latasharorie@hotmail.com`, `haywood.loven@gmail.com`, `mbrewing34@gmail.com`, and `kbrewing02@gmail.com`, and the tester guide now tells people to start in Strategy Locker first, then flow to My Picks for the real confirmation step before they report back. The Test Lab now uses a hybrid guided-pass plus free-roam-pass roadmap so testers can verify core flows and also tell us whether the app gives them enough cues to navigate on their own. The Test Lab popup and main page now also show a 1-week testing window from the tester start date so the guided/free-roam pass is time-bounded. The Test Lab also documents Stripe test-card usage (`4242...` for success, `4000...0002` for a generic decline) so tier upgrades can be exercised without real cards.
- The next Test Lab hardening pass should add a BrewCommand QA dashboard, CA-specific guided coverage, a freshness pre-check, explicit match-chip / hit-vs-win verification, TimePulse refresh / cooldown checks, and a My Picks draw-window filter check so the guided roadmap catches the remaining product-truth gaps without turning them into launch blockers.
- The customer-facing Settings, My Picks, and Profile dropdowns now use branded custom menus instead of browser-native white selects so the visible picker styling matches the rest of BrewLotto.
- BrewCommand admin and QA filter/edit dropdowns now use the same branded custom menu treatment so the operator surfaces match the customer-facing styling instead of showing browser-native white menus.
...
- Customer-facing notifications are still only partially wired: the notification center exists, support updates now land in inbox + email, and winnings/settlement alerts now read from `play_logs` but still need a real cash-value prize-table source before they become fully precise. In-app alerts and email are the live channels today; push and SMS stay as future add-ons until Twilio/mobile delivery are ready. The current copy is being softened so it reads more like a customer update center and less like a technical status panel.
...
- The older odds-driven strategy ideas are now captured in `brewdocs/v1/historical-strategy-ideas.md`, including the heatmap-style odds visualization concept that was meant to teach positional strength and range concentration rather than promise a guaranteed win.
- That historical strategy note now groups the ideas into `now / later / maybe never` buckets so the old odds work stays useful without getting mixed up with the live V1 canon.
- The new strategy product vision page now acts as the decision guide above those notes, so future strategy ideas can be judged by whether they help users understand a pick, respect NC / CA differences, and stay clear of guarantee-style language.
- The shorter strategy decision checklist is now available too, so launch reviews can quickly sort an idea into `Now`, `Later`, or `Maybe never` without rereading the full vision page.
...
8. Build the QA/Test Lab flow for approved testers so the whole app can be exercised tier-by-tier, with yes/no checks, screenshots, and simple notes written into the BrewCommand QA queue. The test copy should explicitly cover TimePulse for BrewPro and TimePulse II for BrewMaster.
9. Normalize the customer notification pipeline so support updates, settlement events, and Brew AI strategy detections can write to `user_notifications` and email the correct BrewLotto return link. Keep the customer-facing copy plain and friendly, and treat in-app/email as the live channels until Twilio adds text delivery.
