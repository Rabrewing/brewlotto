...
- NC Pick 3 / Pick 4 Fireball still needs explicit model handling so the play/result model can account for the modifier instead of treating the draw like a plain straight or box only result.
...
- The midday/evening ingestion schedule needs a live verification pass because the scheduler/auth changes could regress the NC/CA draw-window timing.
...
- The dashboard momentum gauge needs a regression pass because it is now flat or missing across games even though the hot/cold cards still render.
