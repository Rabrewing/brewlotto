...
- BrewU now also has a dedicated Test Lab flow for approved QA testers. Family testers can sign in with approved tester emails, see a gated intro overlay first, then walk the app by tier and submit yes/no + freeform QA reports with screenshots into a separate BrewCommand QA queue backed by `qa_reports` and `qa-screenshots`. Tester identity is auto-captured from the signed-in account so the form can stay simple, and the Test Lab also documents Stripe test-card usage (`4242...` for success, `4000...0002` for a generic decline) so tier upgrades can be exercised without real cards.
