...
import {
  resolveTimingAccessLabel,
  resolveTimingAccessMode,
} from '@/utils/timepulse';
...
export default function StrategyLockerPage() {
  ...
  const [entitlements, setEntitlements] =
    useState<UserEntitlementRecord | null>(null);
  ...
  const [runPreviews, setRunPreviews] = useState<
    Record<string, RunPreviewRecord>
  >({});
  const [savingPredictionId, setSavingPredictionId] = useState<string | null>(
    null
  );
  ...
  const [selectedDrawWindow, setSelectedDrawWindow] = useState<
    'midday' | 'evening'
  >('midday');
  ...
  useEffect(() => {
    ...
    if (
      gameParam &&
      ['pick3', 'pick4', 'cash5', 'powerball', 'mega'].includes(gameParam)
    ) {
  ...
  const gameConfig =
    resolveDashboardGameConfig(selectedGame, preferredStateCode) ||
    ...
    async function loadLocker() {
      ...
      try {
        ...
        if (!authUser) {
          if (!cancelled) {
            setError(
              'You need to sign in before Brew can show your strategy access.'
            );
          }
          return;
        }
        ...
        const [strategiesResult, savedResult] = (await Promise.all([
          supabase
            ...
            .select(
              'id, strategy_key, public_name, description, category, min_tier, sort_order, metadata'
            )
            .eq('is_active', true)
          ...
          QueryResult<StrategyRecord[]>,
          QueryResult<SavedStrategyRecord[]>,
        ];
        ...
        const activityQuery = supabase
          ...
          QueryResult<StrategyActivityRecord[]>
        >;
        ...
        const playLogsQuery = supabase
          .from('play_logs')
          .select(
            'id, prediction_id, outcome_result_code, outcome_match_count, is_settled, created_at, metadata'
          )
          .eq('user_id', authUser.id)
          .order('created_at', { ascending: false })
        ...
        const entitlementsQuery = supabase
          ...
          .select(
            'tier_code, advanced_strategy_access, premium_explanations_access, premium_comparison_access, voice_commentary_access, timing_analysis_access'
          )
          .eq('user_id', authUser.id)
          .maybeSingle() as unknown as Promise<
          QueryResult<UserEntitlementRecord | null>
        >;
        const tiersQuery = supabase
          ...
          QueryResult<SubscriptionTierRecord[]>
        >;
        const [
          activityResult,
          predictionsResult,
          ...
          .map(([label, result]) => {
            if (result.status === 'fulfilled') {
              ...
              return payload.error
                ? `${label}: ${payload.error.message || 'unknown error'}`
                : null;
            }
        ...
        if (!cancelled) {
          const typedStrategiesResult = strategiesResult as QueryResult<
            StrategyRecord[]
          >;
          const typedSavedResult = savedResult as QueryResult<
            SavedStrategyRecord[]
          >;
          const typedActivityResult = activityResult as PromiseFulfilledResult<
            QueryResult<StrategyActivityRecord[]>
          >;
          const typedPredictionsResult =
            predictionsResult as PromiseFulfilledResult<
              QueryResult<PredictionRecord[]>
            >;
          const typedPlayLogsResult = playLogsResult as PromiseFulfilledResult<
            QueryResult<PlayLogRecord[]>
          >;
          const typedEntitlementsResult =
            entitlementsResult as PromiseFulfilledResult<
              QueryResult<UserEntitlementRecord | null>
            >;
          const typedTiersResult = tiersResult as PromiseFulfilledResult<
            QueryResult<SubscriptionTierRecord[]>
          >;
          ...
          setSavedStrategies(
            (typedSavedResult.data || []) as SavedStrategyRecord[]
          );
          setStrategyActivity(
            (typedActivityResult.status === 'fulfilled'
              ? typedActivityResult.value.data || []
              : []) as StrategyActivityRecord[]
          );
          setPredictions(
            (typedPredictionsResult.status === 'fulfilled'
              ? typedPredictionsResult.value.data || []
              : []) as PredictionRecord[]
          );
          setPlayLogs(
            (typedPlayLogsResult.status === 'fulfilled'
              ? typedPlayLogsResult.value.data || []
              : []) as PlayLogRecord[]
          );
          setEntitlements(
            typedEntitlementsResult.status === 'fulfilled'
              ? (typedEntitlementsResult.value
                  .data as UserEntitlementRecord | null) || null
              : null
          );
          setSubscriptionTiers(
            (typedTiersResult.status === 'fulfilled'
              ? typedTiersResult.value.data || []
              : []) as SubscriptionTierRecord[]
          );
          if (optionalErrors.length > 0) {
            console.warn(
              'Strategy Locker loaded with partial data:',
              optionalErrors
            );
          }
        }
        ...
        if (!cancelled) {
          setError(
            loadError instanceof Error
              ? loadError.message
              : 'Failed to load strategy locker'
          );
        }
      } finally {
        if (!cancelled) {
  ...
  const hasTimePulseIIAccess =
    currentTier === 'master' || Boolean(entitlements?.timing_analysis_access);
  ...
  const currentTierRow = useMemo(() => {
    return (
      subscriptionTiers.find((tier) => tier.tier_key === currentTier) || null
    );
  ...
  const nextTierRow = useMemo(() => {
    return (
      subscriptionTiers.find(
        (tier) => TIER_ORDER[tier.tier_key] === currentTierIndex + 1
      ) ||
      subscriptionTiers.find(
        (tier) => TIER_ORDER[tier.tier_key] > currentTierIndex
      ) ||
      null
    );
  ...
  const predictionSummary = useMemo(() => {
    ...
    for (const entry of predictions) {
      ...
      if (
        ...
        Number.isFinite(Number(entry.confidence_score))
      ) {
  ...
  const unlockedCount = useMemo(() => {
    return strategies.filter(
      (entry) => TIER_ORDER[currentTier] >= TIER_ORDER[entry.min_tier]
    ).length;
  }, [currentTier, strategies]);

  const favoriteCount = savedStrategies.filter(
  ...
  async function handleToggleSaved(strategy: StrategyRecord) {
    ...
    try {
      ...
      if (!response.ok || !payload?.success) {
        const message =
          payload?.error?.message || 'Failed to update saved strategy';
        throw new Error(message);
      }

      if (payload.data?.action === 'remove') {
        setSavedStrategies((current) =>
          current.filter((entry) => entry.strategy_id !== strategy.id)
        );
        ...
        setSavedStrategies((current) => {
          ...
          const filtered = current.filter(
            (entry) => entry.strategy_id !== strategy.id
          );
      ...
      setError(
        saveError instanceof Error
          ? saveError.message
          : 'Failed to update saved strategy'
      );
    } finally {
    ...
    try {
      ...
      setRunPreviews((current) => ({
        ...
        [strategy.id]: {
          ...
          primaryNumbers: Array.isArray(payload.data.primaryNumbers)
            ? payload.data.primaryNumbers
            : [],
          bonusNumbers: Array.isArray(payload.data.bonusNumbers)
            ? payload.data.bonusNumbers
            : [],
        ...
        {loading ? (
          ...
          <div className="space-y-5">
              ...
              <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div>
                  ...
                  <div className="mt-2 max-w-2xl text-[15px] leading-7 text-white/62">
                    Strategy Locker shows your current plan, what is unlocked
                    now, and what is still next in line. Expand the ladder only
                ...
                <div className="flex flex-wrap gap-2 text-[12px] uppercase tracking-[0.14em] text-white/42">
                  ...
                  <span className="rounded-full border border-white/8 bg-white/[0.03] px-3 py-1">
                    Activity {strategyActivity.length}
                  </span>
                </div>
              </div>
            </section>
            ...
            <div className="text-[12px] uppercase tracking-[0.16em] text-white/38">
              Choose your game to strategize in
            </div>
            <GameTabs
              ...
              >
                <div className="grid gap-3 md:grid-cols-3">
                  <div className="rounded-[22px] border border-white/8 bg-black/20 px-4 py-4">
                    ...
                    <div className="mt-3 text-[18px] font-medium text-[#f7ddb3]">
                      {currentTierRow?.display_name ||
                        formatTierLabel(currentTier)}
                    </div>
                    <div className="mt-2 text-[13px] text-white/54">
                      {currentTierRow?.price_monthly != null
                        ? `$${Number(currentTierRow.price_monthly).toFixed(2)}/mo`
                        : 'Current tier loaded from entitlements'}
                    </div>
                  </div>
                  <div className="rounded-[22px] border border-white/8 bg-black/20 px-4 py-4">
                    <div className="text-[12px] uppercase tracking-[0.16em] text-white/35">
                    ...
                    <div className="mt-3 text-[18px] font-medium text-[#f7ddb3]">
                      {unlockedCount} strategies
                    </div>
                    <div className="mt-2 text-[13px] text-white/54">
                      {entitlements?.advanced_strategy_access ||
                      entitlements?.premium_explanations_access ||
                      entitlements?.premium_comparison_access ||
                      entitlements?.voice_commentary_access
                  ...
                  <div className="rounded-[22px] border border-white/8 bg-black/20 px-4 py-4">
                    ...
                    <div className="mt-3 text-[18px] font-medium text-[#f7ddb3]">
                      {nextTierRow?.display_name || 'No higher tier'}
                    </div>
                    <div className="mt-2 text-[13px] text-white/54">
                ...
                <div className="mt-4 flex flex-wrap items-center gap-3">
                  ...
                  <div className="text-[12px] uppercase tracking-[0.14em] text-white/38">
                    {remainingUpgradeTiers.length > 0
                      ? `${remainingUpgradeTiers.length} tier${remainingUpgradeTiers.length === 1 ? '' : 's'} above current plan`
                      : 'No higher tiers left'}
                  </div>
                </div>
                      ...
                      <div className="mt-3 grid gap-3 sm:grid-cols-2">
                        ...
                        <div className="rounded-[18px] border border-[#72caff]/18 bg-[#111f28] px-4 py-3">
                          ...
                          <div className="mt-2 text-[16px] font-medium text-[#d7ecff]">
                            {hasTimePulseAccess ? 'Enabled' : 'Locked'}
                          </div>
                          <div className="mt-1 text-[12px] text-white/40">
                            BrewPro timing window and lag profile
                          </div>
                        </div>
                        <div className="rounded-[18px] border border-[#ffbd39]/18 bg-[#1a140c] px-4 py-3">
                          ...
                          <div className="mt-2 text-[16px] font-medium text-[#f7ddb3]">
                            {hasTimePulseIIAccess ? 'Enabled' : 'Locked'}
                          </div>
                          <div className="mt-1 text-[12px] text-white/40">
                            BrewMaster adaptive timing and regime shifts
                          </div>
                        </div>
                      </div>
                    </div>
                            ...
                            >
                              <div className="flex items-center justify-between gap-3">
                                <div>
                                  ...
                                  <div className="mt-2 text-[18px] font-medium text-[#f7ddb3]">
                                    {tier.display_name}
                                  </div>
                                </div>
            ...
            >
              <div className="grid gap-4 lg:grid-cols-2">
                {strategies.map((strategy) => {
                  const hasAccess =
                    TIER_ORDER[currentTier] >= TIER_ORDER[strategy.min_tier];
                  const saved = savedMap.get(strategy.id);
                  const activity = activityMap.get(strategy.id) || [];
                  const predictionData = predictionSummary.get(
                    strategy.strategy_key
                  );
                  const performanceData = strategyPerformanceMap.get(
                    getStrategyLabel(strategy.strategy_key)
                  );
                  const lastUsedAt =
                    activity[0]?.occurred_at ||
                    predictionData?.lastUsedAt ||
                    null;
                  const averageConfidence =
                    predictionData && predictionData.confidenceCount > 0
                      ? Math.round(
                          predictionData.confidenceTotal /
                            predictionData.confidenceCount
                        )
                      : null;
                    ...
                    >
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <div className="flex flex-wrap items-center gap-2">
                            <div
                              className={`text-[19px] font-medium ${hasAccess ? 'text-[#f7ddb3]' : 'text-white/72'}`}
                            >
                              {getStrategyLabel(strategy.strategy_key)}
                            </div>
                          ...
                          <div className="mt-3 text-[14px] leading-7 text-white/58">
                            {strategy.description ||
                              'No strategy description stored yet.'}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                        ...
                        <div className="rounded-[18px] border border-white/8 bg-white/[0.03] px-4 py-3">
                          ...
                          <div className="mt-2 text-[16px] font-medium text-[#f7ddb3]">
                            {performanceData?.confirmedPlays || 0}
                          </div>
                        </div>
                        <div className="rounded-[18px] border border-white/8 bg-white/[0.03] px-4 py-3">
                          <div className="text-[11px] uppercase tracking-[0.14em] text-white/35">
                          ...
                          <div className="mt-2 text-[16px] font-medium text-[#f7ddb3]">
                            {performanceData?.hitRate !== null &&
                            performanceData?.hitRate !== undefined
                        ...
                        <div className="rounded-[18px] border border-white/8 bg-white/[0.03] px-4 py-3">
                          ...
                          <div className="mt-2 text-[16px] font-medium text-[#f7ddb3]">
                            {performanceData?.winRate !== null &&
                            performanceData?.winRate !== undefined
                              ? `${performanceData.winRate}%`
                              : 'N/A'}
                          </div>
                        </div>
                        <div className="rounded-[18px] border border-[#72caff]/18 bg-[#111f28] px-4 py-3">
                          <div className="text-[11px] uppercase tracking-[0.14em] text-[#9edcff]">
                            Fireball plays
                          </div>
                          <div className="mt-2 text-[16px] font-medium text-[#d7ecff]">
                            {performanceData?.fireballConfirmedPlays || 0}
                          </div>
                        </div>
                        <div className="rounded-[18px] border border-[#72caff]/18 bg-[#111f28] px-4 py-3">
                          <div className="text-[11px] uppercase tracking-[0.14em] text-[#9edcff]">
                          ...
                          <div className="mt-2 text-[16px] font-medium text-[#d7ecff]">
                            {performanceData?.fireballHitRate !== null &&
                            performanceData?.fireballHitRate !== undefined
                              ? `${performanceData.fireballHitRate}%`
                              : 'N/A'}
                          </div>
                        </div>
                        <div className="rounded-[18px] border border-[#72caff]/18 bg-[#111f28] px-4 py-3">
                          ...
                          <div className="mt-2 text-[16px] font-medium text-[#d7ecff]">
                            {performanceData?.fireballWinRate !== null &&
                            performanceData?.fireballWinRate !== undefined
                              ? `${performanceData.fireballWinRate}%`
                              : 'N/A'}
                          </div>
                        </div>
                        <div className="rounded-[18px] border border-white/8 bg-white/[0.03] px-4 py-3">
                      ...
                      <div className="mt-4 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                        <div
                          className={`inline-flex w-fit items-center rounded-full px-3 py-1 text-[11px] uppercase tracking-[0.14em] ${hasAccess ? 'border border-[#72caff]/18 bg-[#72caff]/10 text-[#9fdcff]' : 'border border-[#ff8d7b]/18 bg-[#ff8d7b]/10 text-[#ffc4b8]'}`}
                        >
                          {hasAccess
                            ? 'Unlocked'
                            : `Requires ${formatTierLabel(strategy.min_tier)}`}
                        </div>
                        <div className="flex flex-wrap gap-3">
                          <button
                            type="button"
                            disabled={
                              !hasAccess || savingStrategyId === strategy.id
                            }
                          ...
                          <button
                            type="button"
                            disabled={
                              !hasAccess || runningStrategyId === strategy.id
                            }
                      ...
                      {runPreviews[strategy.id] ? (
                        <div className="mt-4 rounded-[22px] border border-[#72caff]/16 bg-[#72caff]/8 px-4 py-4">
                          ...
                          <div className="mt-3 text-[14px] text-white/72">
                            {getStrategyLabel(
                              runPreviews[strategy.id].engineKey
                            )}{' '}
                            ran on {runPreviews[strategy.id].homeState}{' '}
                            {runPreviews[strategy.id].gameKey.toUpperCase()}.
                          </div>
                          <div className="mt-3 text-[20px] font-medium text-[#f7ddb3]">
                            {runPreviews[strategy.id].primaryNumbers.join(' ')}
                            {runPreviews[strategy.id].bonusNumbers.length > 0
                              ? ` + ${runPreviews[strategy.id].bonusNumbers.join(' ')}`
                              : ''}
                          </div>
                          ...
                          {runPreviews[strategy.id].timingProfile ? (
                            (() => {
                              ...
                              return (
                                <div
                                  className={`mt-3 rounded-[18px] px-4 py-3 ${timingMode === 'master' ? 'border border-[#ffbd39]/14 bg-[#1a140c]' : 'border border-[#72caff]/14 bg-[#111f28]'}`}
                                >
                                  <div className="flex items-center gap-2">
                                    <span
                                      className={`text-[12px] font-medium ${timingMode === 'master' ? 'text-[#f5cf84]' : 'text-[#9edcff]'}`}
                                    >
                                      {runPreviews[strategy.id]
                                        .timingTierLabel ||
                                        (timingMode === 'master'
                                    ...
                                    <span
                                      className={`ml-auto rounded-full px-2 py-0.5 text-[10px] font-medium uppercase tracking-[0.1em] ${
                                  ...
                                  <div className="mt-2 text-[13px] leading-6 text-white/72">
                                    ...
                                    — best play window:{' '}
                                    {timingProfile.windowStart} to{' '}
                                    {timingProfile.windowEnd}
                                  </div>
                                  <div className="mt-1 text-[11px] text-white/40">
                                    Based on {timingProfile.sampleSize}{' '}
                                    historical{' '}
                                    {getStrategyLabel(
                                      runPreviews[strategy.id].engineKey
                                    )}{' '}
                                    patterns • median {timingProfile.median}{' '}
                                    days • {timingProfile.spread}d spread
                                  </div>
                                </div>
                              );
                            })()
                          ) : timingAccessMode &&
                            runPreviews[strategy.id].predictionId ? (
                            <div
                              className={`mt-3 rounded-[18px] px-4 py-3 ${timingAccessMode === 'master' ? 'border border-[#ffbd39]/12 bg-[#1a140c]/60' : 'border border-[#72caff]/12 bg-[#111f28]/60'}`}
                            >
                              <div className="flex items-center gap-2">
                                <span
                                  className={`text-[12px] font-medium ${timingAccessMode === 'master' ? 'text-[#f5cf84]' : 'text-[#9edcff]'}`}
                                >
                                  {runPreviews[strategy.id].timingTierLabel ||
                          ...
                          {runPreviews[strategy.id].strategyComparisons
                            ? (function () {
                                ...
                                const entries = Object.entries(
                                  ...
                                  return e[1] && e[1].sampleSize >= 3;
                                });
                                if (entries.length === 0) return null;
                                entries.sort(function (a, b) {
                                  return (
                                    a[1].p75 - a[1].p25 - (b[1].p75 - b[1].p25)
                                  );
                                });
                                ...
                                const currentTimingProfile =
                                  runPreviews[strategy.id].timingProfile;
                                const currentSpread = currentTimingProfile
                                  ? currentTimingProfile.p75 -
                                    currentTimingProfile.p25
                                  : null;
                                ...
                                const styleHint =
                                  runPreviews[strategy.id].timingProfile
                                    ?.recommendedStyle;
                                const styleLabel = styleHint
                                  ? styleMap[styleHint] || null
                                  : null;

                                const currentLabel = getStrategyLabel(
                                  strategy.strategy_key
                                );
                                return (
                                  <div className="mt-2 rounded-[18px] border border-[#72caff]/14 bg-[#101922] px-4 py-3">
                                    <div className="text-[11px] font-medium text-[#9edcff]">
                                      Brew AI
                                    </div>
                                    <div className="mt-1 text-[13px] leading-6 text-white/72">
                                      {best[0] !== currentLabel &&
                                      spread < (currentSpread || 99)
                                      ...
                                      {styleLabel ? (
                                        <span className="mt-1 block text-[#93efb8]">
                                          Historical pattern favors a{' '}
                                          <strong>{styleLabel}</strong> play
                                          style for this game.
                                        </span>
                                      ) : null}
                                    </div>
                          ...
                          <div className="mt-2 text-[12px] uppercase tracking-[0.14em] text-white/40">
                            Stored at{' '}
                            {new Date(
                              runPreviews[strategy.id].createdAt
                            ).toLocaleString()}
                          </div>
                          {!runPreviews[strategy.id].predictionSaved ? (
                            ...
                            >
                              {savingPredictionId === strategy.id
                                ? 'Saving...'
                                : 'Save to My Picks'}
                            </button>
                          ) : (
                            <div className="mt-3 text-[13px] font-medium text-[#93efb8]">
                              ✓ Saved to My Picks
                            </div>
                          )}
                        </div>
