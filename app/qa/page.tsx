...
const TIER_OPTIONS: Array<{
...
}> = [
  {
    value: 'free',
    label: 'Free',
    description: 'Dashboard showcase, onboarding, and read-only surfaces.',
  },
  {
    value: 'starter',
    label: 'Starter',
    description: 'Strategy Locker entry with a small set of strategies.',
  },
  {
    value: 'pro',
    label: 'Pro',
    description:
      'Expanded strategy flow, confirmation, tracking, and TimePulse.',
  },
  {
    value: 'master',
    label: 'Master',
    description: 'TimePulse II adaptive timing and lag-based ranges.',
  },
];
...
const JOURNEY_STAGES: Array<{
...
}> = [
  {
    value: 'start_here',
    label: 'Start here',
    description: 'Login, onboarding, and the first dashboard view.',
  },
  {
    value: 'onboarding',
    label: 'Onboarding',
    description: 'Disclaimer, tutorial, and first-run setup.',
  },
  {
    value: 'dashboard_free',
    label: 'Free dashboard',
    description: 'Read-only home flow and app showcase.',
  },
  {
    value: 'starter_strategy_locker',
    label: 'Starter locker',
    description: 'Run a strategy and save a pick.',
  },
  {
    value: 'my_picks_confirmation',
    label: 'My Picks',
    description: 'Confirm a saved play and check Fireball.',
  },
  {
    value: 'results_history',
    label: 'Results',
    description: 'Draw history, dates, and match clarity.',
  },
  {
    value: 'billing_pricing',
    label: 'Billing / Pricing',
    description: 'Upgrade, downgrade, and Stripe checkout.',
  },
  {
    value: 'notifications',
    label: 'Notifications',
    description: 'Inbox, nudges, and delivery preferences.',
  },
  {
    value: 'support',
    label: 'Support',
    description: 'Report an issue and attach a screenshot.',
  },
  {
    value: 'brewu',
    label: 'BrewU',
    description: 'Help hub, play styles, and payout guidance.',
  },
  {
    value: 'master_timepulse',
    label: 'Master / TimePulse II',
    description: 'Timing window, lag range, and Master-only access.',
  },
  {
    value: 'other',
    label: 'Other',
    description: 'Anything else the tester ran into.',
  },
];

const FEATURE_AREAS: Array<{
  value: FeatureArea;
  label: string;
  description: string;
}> = [
  {
    value: 'dashboard',
    label: 'Dashboard',
    description: 'Home surface and free-tier showcase.',
  },
  {
    value: 'strategy-locker',
    label: 'Strategy Locker',
    description: 'Run / save / entitlement flow.',
  },
  {
    value: 'my-picks',
    label: 'My Picks',
    description: 'Saved picks and play confirmation.',
  },
  {
    value: 'results',
    label: 'Results',
    description: 'Date-based draw history and matches.',
  },
  {
    value: 'stats',
    label: 'Stats',
    description: 'Hit rates, wins, and performance.',
  },
  {
    value: 'pricing',
    label: 'Pricing',
    description: 'Plan selection and upgrade state.',
  },
  {
    value: 'billing',
    label: 'Billing',
    description: 'Portal, checkout, and subscription state.',
  },
  {
    value: 'notifications',
    label: 'Notifications',
  ...
  },
  {
    value: 'support',
    label: 'Support',
    description: 'Issue intake and screenshot upload.',
  },
  {
    value: 'learn',
    label: 'BrewU',
    description: 'FAQ, play styles, and strategy help.',
  },
  {
    value: 'login',
    label: 'Login / Auth',
    description: 'Magic link and onboarding entry.',
  },
  {
    value: 'other',
    label: 'Other',
    description: 'Something outside the list above.',
  },
];
...
export default function QaPage() {
  ...
  useEffect(() => {
    try {
      setIntroAccepted(
        window.localStorage.getItem(QA_INTRO_STORAGE_KEY) === 'true'
      );
  ...
  const selectedTier = useMemo(
    () =>
      TIER_OPTIONS.find((entry) => entry.value === formState.tierTested) ||
      TIER_OPTIONS[0],
    [formState.tierTested]
  );
  const selectedStage = useMemo(
    () =>
      JOURNEY_STAGES.find((entry) => entry.value === formState.journeyStage) ||
      JOURNEY_STAGES[0],
    [formState.journeyStage]
  );
  const selectedArea = useMemo(
    () =>
      FEATURE_AREAS.find((entry) => entry.value === formState.featureArea) ||
      FEATURE_AREAS[0],
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSubmissionMessage(
        'QA report sent. BrewCommand will review it and track the next step.'
      );
      ...
      <main className="min-h-screen bg-[#050505] text-white">
        <DashboardContainer>
          ...
          <div className="mt-8 rounded-[28px] border border-[#ff8d7b]/25 bg-[#2a120d]/60 px-5 py-8 text-center text-[#ffc4b8]">
            Test Lab access is only available to approved BrewCommand admin and
            QA tester accounts.
          </div>
        </DashboardContainer>
      </main>
    ...
    <main className="min-h-screen bg-[#050505] text-white">
      <DashboardContainer>
        ...
        {!introAccepted ? (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 px-3 py-4 backdrop-blur-md sm:px-4 sm:py-6">
            <div className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-[30px] border border-[#72caff]/25 bg-[radial-gradient(circle_at_top_left,rgba(114,202,255,0.22),rgba(0,0,0,0)_32%),linear-gradient(145deg,rgba(14,20,28,0.98),rgba(6,6,6,0.98))] p-4 shadow-[0_0_60px_rgba(114,202,255,0.18)] sm:p-8">
              <div className="text-[12px] uppercase tracking-[0.18em] text-[#bde7ff]">
                Before you start
              </div>
              <div className="mt-3 text-[24px] font-semibold tracking-[-0.03em] text-[#d8f1ff] sm:text-[30px]">
                Read this first so we capture the full test flow
              </div>
              <div className="mt-3 max-w-2xl text-[14px] leading-7 text-white/68 sm:text-[15px]">
                This Test Lab is for approved family testers. It uses two
                passes: first a guided pass so we can verify the core flow, then
              ...
              <div className="mt-5 rounded-[22px] border border-[#72caff]/18 bg-[#101922] px-4 py-4">
                <div className="text-[12px] uppercase tracking-[0.16em] text-[#bde7ff]">
                  Free-roam pass
                </div>
                <div className="mt-2 text-[14px] leading-7 text-white/68">
                  After the guided pass, try to find the important surfaces
                  without instructions and tell us whether the app gave you
                  enough clues.
                </div>
              ...
              <div className="mt-5 rounded-[22px] border border-[#ffc742]/18 bg-[#20170a] px-4 py-4 text-[13px] leading-6 text-[#f7ddb3] sm:text-[14px] sm:leading-7">
                <div className="text-[12px] uppercase tracking-[0.16em] text-[#ffd873]">
                  Testing window
                </div>
                <div className="mt-2">
                  You have{' '}
                  <span className="font-semibold text-white">7 days</span> from
                  your first entry to test.
                </div>
                <div className="mt-2 text-white/74">
                  Start date:{' '}
                  {qaStartDate
                    ? formatQaDateTime(qaStartDate)
                    : 'It starts when you click Begin Test Lab.'}
                </div>
                {qaStartDate ? (
              ...
              <div className="mt-5 rounded-[22px] border border-white/10 bg-black/20 px-4 py-4 text-[13px] leading-6 text-white/66 sm:text-[14px] sm:leading-7">
                If you reach pricing or billing, check that pricing shows the
                correct tier ladder and that Billing opens the manage surface.
                Use Stripe test cards only. For a normal successful checkout,
                use <span className="text-[#d8f1ff]">4242 4242 4242 4242</span>{' '}
                with any future expiry date like{' '}
                <span className="text-[#d8f1ff]">09/30</span> and any 3-digit
                CVC like <span className="text-[#d8f1ff]">222</span>. If you
                need to test a decline path, use Stripe&apos;s generic decline
                card <span className="text-[#d8f1ff]">4000 0000 0000 0002</span>{' '}
                with any future expiry date like{' '}
                <span className="text-[#d8f1ff]">09/30</span> and any 3-digit
                CVC like <span className="text-[#d8f1ff]">222</span> instead of
                a real card.
              </div>
        ...
        <section className="rounded-[30px] border border-[#72caff]/24 bg-[radial-gradient(circle_at_top_left,rgba(114,202,255,0.18),rgba(0,0,0,0)_34%),linear-gradient(145deg,rgba(18,24,34,0.9),rgba(8,8,8,0.98))] px-5 py-5 shadow-[0_0_28px_rgba(114,202,255,0.08)]">
          <div className="text-[15px] uppercase tracking-[0.16em] text-[#bde7ff]">
            QA / tester workflow
          </div>
          <div className="mt-3 text-[26px] font-semibold text-[#d8f1ff]">
            Walk the app from sign-in to tier testing, then tell us what
            happened
          </div>
          <div className="mt-2 max-w-3xl text-[15px] leading-7 text-white/62">
            This page is for approved testers only. Start at the beginning, move
            through each tier, then do a second free-roam pass to see whether
            the app teaches itself clearly enough. Use the form below to report
            what happened versus what should have happened. Your report will
            land in BrewCommand, not customer support.
          </div>
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <div className="rounded-[20px] border border-[#ffc742]/18 bg-[#20170a] px-4 py-4 text-[13px] leading-6 text-[#f7ddb3]">
              <div className="text-[11px] uppercase tracking-[0.16em] text-[#ffd873]">
                Testing window
              </div>
              <div className="mt-2 text-[14px] text-white/82">
                You have{' '}
                <span className="font-semibold text-white">7 days</span> from
                your first entry to finish the guided and free-roam passes.
              </div>
            </div>
            <div className="rounded-[20px] border border-[#72caff]/18 bg-[#101922] px-4 py-4 text-[13px] leading-6 text-[#d8f1ff]">
              <div className="text-[11px] uppercase tracking-[0.16em] text-[#bde7ff]">
                Start date
              </div>
              <div className="mt-2 text-[14px] text-white/82">
                {qaStartDate
          ...
          <div className="mt-4 flex flex-wrap gap-2 text-[12px] text-white/58">
            <span className="rounded-full border border-white/10 bg-white/[0.03] px-3 py-1.5">
              Start at login / onboarding
            </span>
            <span className="rounded-full border border-white/10 bg-white/[0.03] px-3 py-1.5">
              Test each tier in order
            </span>
            <span className="rounded-full border border-white/10 bg-white/[0.03] px-3 py-1.5">
              Do a free-roam pass after
            </span>
            <span className="rounded-full border border-white/10 bg-white/[0.03] px-3 py-1.5">
              Check the free-tier countdown banner
            </span>
            <span className="rounded-full border border-white/10 bg-white/[0.03] px-3 py-1.5">
              Verify TimePulse / TimePulse II
            </span>
            <span className="rounded-full border border-white/10 bg-white/[0.03] px-3 py-1.5">
              Use fake Stripe test cards only
            </span>
            <span className="rounded-full border border-white/10 bg-white/[0.03] px-3 py-1.5">
              Reports go to BrewCommand
            </span>
          </div>
          <div className="mt-3 max-w-3xl text-[13px] leading-6 text-white/48">
            If you reach billing, use Stripe test cards only. The common test
            number is{' '}
            <span className="text-[#d8f1ff]">4242 4242 4242 4242</span> with any
            future expiry date like{' '}
            <span className="text-[#d8f1ff]">09/30</span> and any 3-digit CVC
            like <span className="text-[#d8f1ff]">222</span>.
          </div>
          <div className="mt-5 flex flex-wrap gap-3">
            <Link
              href="/onboarding"
              className="rounded-full border border-[#72caff]/20 bg-[#111f28] px-4 py-2 text-sm font-semibold text-[#d8f1ff] transition hover:border-[#72caff]/35 hover:bg-[#162732]"
            >
              Open onboarding
            </Link>
            <Link
              href="/dashboard"
              className="rounded-full border border-[#ffd978]/20 bg-[#20170a] px-4 py-2 text-sm font-semibold text-[#ffd873] transition hover:border-[#ffd978]/35 hover:bg-[#2b2210]"
            >
              Start at dashboard
            </Link>
            <Link
              href="/pricing"
              className="rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-sm font-semibold text-white/72 transition hover:border-white/20 hover:bg-white/10"
            >
              Check pricing
            </Link>
          </div>
        ...
        <section className="mt-5 grid gap-4 xl:grid-cols-2">
          <SectionCard
            title="How to test"
            description="Guided pass first, then a free-roam pass to check whether the app explains itself."
          >
            <div className="space-y-3">
              {GUIDED_TEST_STEPS.map((step, index) => (
                ...
                >
                  ...
                  <div className="mt-2 text-[15px] leading-7 text-white/74">
                    {step}
                  </div>
                </div>
              ))}
            ...
            <div className="mt-5 rounded-[20px] border border-[#72caff]/18 bg-[#101922] p-4">
              ...
              <div className="mt-3 space-y-2">
                {FREE_ROAM_PROMPTS.map((prompt) => (
                  <div
                    key={prompt}
                    className="rounded-[16px] border border-white/8 bg-black/20 px-3 py-2 text-[14px] leading-6 text-white/68"
                  >
                    {prompt}
                  </div>
                ))}
          ...
          <SectionCard
            title="Tier notes"
            description="What each tester should look for at each level, including cues and next steps."
          >
        ...
        <section className="mt-5 grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
          <SectionCard
            ...
            description="Use simple yes / no answers plus one short note about what happened."
          >
            ...
            <div className="mb-5 rounded-[20px] border border-[#72caff]/18 bg-[#0e1720] px-4 py-4">
              ...
              <div className="mt-2 text-[14px] leading-7 text-white/68">
                Your sign-in email is captured automatically. You can confirm or
                tweak the display name below so BrewCommand can match your
                report to the right tester.
              </div>
            ...
            <div className="mb-5 rounded-[20px] border border-white/8 bg-black/20 px-4 py-4">
              ...
              <div className="mt-2 text-[14px] leading-7 text-white/62">
                If something feels off, say where you were, what you expected,
                and what actually happened. Use the priority that matches the
                impact: blocker, major, medium, minor, or polish.
              </div>
            </div>
            <div className="grid gap-3 sm:grid-cols-3">
              ...
              <YesNoToggle
                ...
                onChange={(next) =>
                  setFormState((current) => ({ ...current, tierMatched: next }))
                }
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                <label
                  className="block text-[12px] uppercase tracking-[0.16em] text-white/38"
                ...
                <select
                  ...
                  onChange={(event) =>
                    setFormState((current) => ({
                      ...current,
                      journeyStage: event.target.value as JourneyStage,
                    }))
                  }
                ...
                >
                  {JOURNEY_STAGES.map((stage) => (
                    <option
                      key={stage.value}
                      value={stage.value}
                      className="bg-[#101922] text-[#f7f2e8]"
                    >
                      {stage.label}
                    </option>
                  ))}
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setFormState((current) => ({
                      ...current,
                      featureArea: event.target.value as FeatureArea,
                    }))
                  }
                  className={BRANDED_SELECT_CLASS}
                >
                  ...
                  onChange={(event) =>
                    setFormState((current) => ({
                      ...
                      priority: event.target
                        .value as QaReportFormState['priority'],
                    }))
                  }
                  className={BRANDED_SELECT_CLASS}
                >
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
              <button
                ...
                onClick={() =>
                  setFormState((current) => ({
                    ...current,
                    fireballRelevant: !current.fireballRelevant,
                  }))
                }
              ...
              <button
                ...
                onClick={() =>
                  setFormState((current) => ({
                    ...current,
                    timepulseRelevant: !current.timepulseRelevant,
                  }))
                }
              ...
              <button
                ...
                onClick={() =>
                  setFormState((current) => ({
                    ...current,
                    loadedAsExpected: !current.loadedAsExpected,
                  }))
                }
              ...
              <button
                ...
                onClick={() =>
                  setFormState((current) => ({
                    ...current,
                    tierMatched: !current.tierMatched,
                  }))
                }
              ...
              <button
                ...
                onClick={() =>
                  setFormState((current) => ({
                    ...current,
                    nextStepMatched: !current.nextStepMatched,
                  }))
                }
            ...
            <div className="mt-5 space-y-2">
              ...
              <div className="text-[12px] uppercase tracking-[0.14em] text-white/32">
                Up to 4 screenshots. Attach one if it helps explain what
                happened.
              </div>
            </div>

            <div className="mt-5 rounded-[20px] border border-white/8 bg-black/20 px-4 py-4">
              <div className="text-[13px] font-medium text-[#d8f1ff]">
              ...
              <div className="mt-2 grid gap-2 text-[13px] text-white/60 sm:grid-cols-2">
                ...
                <div>
                  Browser: {String(meta.browserInfo.userAgent || 'Unknown')}
                </div>
                <div>
                  Viewport: {String(meta.browserInfo.viewport || 'Unknown')}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <div className="rounded-[20px] border border-white/10 bg-black/20 p-4">
                ...
                <div className="mt-2 text-[15px] leading-7 text-white/72">
                  Dashboard, BrewU, Results, Support, and the trial / upgrade
                  prompts. Check whether the page explains what to do next.
                </div>
              </div>
              <div className="rounded-[20px] border border-white/10 bg-black/20 p-4">
                <div className="text-[11px] uppercase tracking-[0.16em] text-white/40">
                ...
                <div className="mt-2 text-[15px] leading-7 text-white/72">
                  Strategy Locker entry, run strategy, save to My Picks, and
                  confirm the play. Check whether the app makes the save /
                  confirm path obvious.
                </div>
              </div>
              <div className="rounded-[20px] border border-white/10 bg-black/20 p-4">
                <div className="text-[11px] uppercase tracking-[0.16em] text-white/40">
                ...
                <div className="mt-2 text-[15px] leading-7 text-white/72">
                  Expanded strategy flow, Fireball on NC Pick 3 / Pick 4,
                  TimePulse, and richer stats / ratios. Check whether the extra
                  detail still reads clearly.
                </div>
              </div>
              <div className="rounded-[20px] border border-white/10 bg-black/20 p-4">
                <div className="text-[11px] uppercase tracking-[0.16em] text-white/40">
                ...
                <div className="mt-2 text-[15px] leading-7 text-white/72">
                  TimePulse and TimePulse II timing analysis, lag range
                  guidance, and tier-specific review surfaces. Check whether the
                  Master view feels clearly deeper than the Pro view.
                </div>
              </div>
            </div>
          </SectionCard>
