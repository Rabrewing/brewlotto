                    ...
                    <div className="flex flex-col md:flex-row justify-center gap-4">
                        {["day", "evening"].map((type) => (
                            <div key={type} className="bg-[#232323] rounded-xl flex-1 p-4 shadow border-l-4 border-[#FFD700]">
                                ...
                                <div className="text-lg text-[#FFD700] font-extrabold mb-1">{latest?.[type as keyof LatestDraws]?.result || '-- -- -- -- --'}</div>
                                <div className="text-xs text-gray-500">{latest?.[type as keyof LatestDraws]?.date || '-'}</div>
                            </div>
                        ))}
                    </div>
                )}
