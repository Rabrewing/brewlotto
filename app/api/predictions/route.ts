      ...
      if (gameRow) {
        ...
        if (draws && draws.length > 0) {
          ...
          const predictedNumbers = Array.isArray(prediction.predicted_numbers)
            ? prediction.predicted_numbers.filter(
                (v: unknown): v is number => typeof v === 'number'
              )
            : [];
          const drawNumbers = Array.isArray(draw.primary_numbers)
            ? draw.primary_numbers.filter(
                (v: unknown): v is number => typeof v === 'number'
              )
            : [];
          const drawBonus = Array.isArray(draw.bonus_numbers)
            ? draw.bonus_numbers[0]
            : null;
          const predictedBonus = prediction.bonus_number
            ? Number(prediction.bonus_number)
            : null;
          const drawFireball =
            typeof draw.fireball_value === 'number'
              ? draw.fireball_value
              : null;
          ...
          const bonusMatch =
            drawBonus !== null &&
            predictedBonus !== null &&
            Number(drawBonus) === predictedBonus;
...
function getWorstFreshnessStatus(statuses: string[]) {
  ...
  return (
    [...statuses].sort(
      (a, b) =>
        (rank[a as keyof typeof rank] ?? 5) -
        (rank[b as keyof typeof rank] ?? 5)
    )[0] || 'unknown'
  );
}
  ...
  return (
    entries
      .map((gameId) =>
        resolveDashboardGameConfig(
          gameId,
          normalizedState as DashboardStateCode
        )
      )
      .filter(
        (
          config
        ): config is NonNullable<
          ReturnType<typeof resolveDashboardGameConfig>
        > => Boolean(config)
      )
      .find((config) => {
        const matchesRequestRoute =
          config.requestGameKey === gameKey && config.requestState === state;
        const matchesPredictionRoute =
          config.predictionGame === gameKey &&
          config.predictionStates.includes(state);
...
export async function POST(request: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          ...
          error: {
            ...
            message: 'gameKey and state are required',
          },
        },
        { status: 400 }
      );
    ...
    if (freshnessConfig) {
      ...
      if (freshnessError) {
        return NextResponse.json(
          {
            success: false,
            error: { code: 'FRESHNESS_ERROR', message: freshnessError.message },
          },
          { status: 500 }
        );
      }

      const freshnessStatus = getWorstFreshnessStatus(
        (freshnessRows || []).map((row) => row.freshness_status || 'unknown')
      );
    ...
    const { generatePrediction } =
      await import('@/lib/prediction/predictionGenerator.js');
    ...
    if (!result.success) {
      return NextResponse.json(
        {
          success: false,
          error: { code: 'PREDICTION_ERROR', message: result.error },
        },
        { status: 400 }
      );
    }
