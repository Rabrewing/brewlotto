...
function normalizeTimingGame(game: string | null) {
  const normalized = String(game || '')
    .trim()
    .toLowerCase();
  if (normalized === 'daily3') return 'pick3';
  if (normalized === 'daily4') return 'pick4';
  if (normalized === 'fantasy5') return 'cash5';
  if (normalized === 'mega' || normalized === 'mega_millions') return 'mega';
  return normalized;
}

function normalizeTimingState(state: string | null) {
  return state === 'CA' ? 'CA' : 'NC';
}

function getTimingComboKey(game: string | null, state: string | null) {
  return `${normalizeTimingGame(game)}:${normalizeTimingState(state)}`;
}
...
function getTimingProfileLookupKey(
  ...
  return `${getTimingComboKey(game, state)}:${profileKey}`;
}

function getTimingProfileRefreshKey(game: string | null, state: string | null) {
  return getTimingComboKey(game, state);
}

function getGameAndStateForTiming(prediction: PredictionRecord) {
  return {
    game: normalizeTimingGame(prediction.game),
    state: normalizeTimingState(prediction.state),
  };
}
...
function getFireballContext(playLog?: PlayLogRecord | null) {
  ...
  if (
    typeof playLog.outcome_result_code === 'string' &&
    playLog.outcome_result_code.toLowerCase().includes('fireball')
  ) {
...
function SummaryMetric({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex-1 px-4 py-3 text-center">
      ...
      <div className="mt-2 text-[24px] font-semibold text-[#ffd27e]">
        {value}
      </div>
    </div>
  );
...
function PickCard({
  ...
  const numbers = Array.isArray(prediction.predicted_numbers)
  ...
  return (
    <article className="rounded-[28px] border border-[#ffbd39]/22 bg-[linear-gradient(145deg,rgba(32,19,13,0.82),rgba(13,10,10,0.96))] px-5 py-4 shadow-[0_0_22px_rgba(255,184,28,0.08)]">
      ...
      <div className="mt-4 text-[14px] leading-7 text-white/66">
...
export default function MyPicksPage() {
  ...
  const [confirmedPredictionIds, setConfirmedPredictionIds] = useState<
    string[]
  >([]);
  ...
  const [timingRefreshCooldowns, setTimingRefreshCooldowns] = useState<
      ...
      try {
        ...
        const [predictionsResponse, playLogsResponse] = (await Promise.all([
          ...
          supabase
            .from('play_logs')
            .select(
              'id, prediction_id, is_settled, outcome_result_code, outcome_match_count, draw_date, created_at, metadata'
            )
            .eq('user_id', userId)
        ...
        const typedPlayLogsResponse = playLogsResponse as QueryResult<
          PlayLogRecord[]
        >;
        ...
        if (!cancelled) {
          setPredictions(data);
          const nextPlayLogs = (typedPlayLogsResponse.data ||
            []) as PlayLogRecord[];
  ...
  const filteredPredictions = useMemo(() => {
    return predictions.filter((prediction) => {
      ...
      const windowMatches =
        selectedWindow === 'ALL' ||
        normalizeDrawWindow(prediction.draw_time ?? null) === selectedWindow;
      return gameMatches && windowMatches;
    });
  }, [predictions, selectedGame, selectedWindow]);

  const timingCombos = useMemo(() => {
    ...
    for (const prediction of filteredPredictions) {
      ...
      combos.set(getTimingComboKey(combo.game, combo.state), combo);
    }

    if (combos.size === 0) {
      const fallbackState = selectedState === 'ALL' ? 'NC' : selectedState;
      const fallbackGame =
        selectedGame === 'ALL' ? 'pick3' : normalizeTimingGame(selectedGame);
      combos.set(getTimingComboKey(fallbackGame, fallbackState), {
        game: fallbackGame,
        state: fallbackState,
      });
    }
  ...
  }, [filteredPredictions, selectedGame, selectedState]);

  useEffect(() => {
    let cancelled = false;

    async function loadTimingProfiles() {
      const mode = currentTier === 'master' ? 'master' : 'pro';
      const nextProfiles: Record<string, TimingProfileRecord> = {};

      for (const combo of timingCombos) {
        const comboKey = getTimingProfileRefreshKey(combo.game, combo.state);
        try {
          const res = await fetch(
            `/api/stats/timing?game=${combo.game}&state=${combo.state}&mode=${mode}`,
            {
              cache: 'no-store',
            }
          );
          ...
          for (const [label, profile] of Object.entries(
            payload.data as Record<string, TimingProfileRecord>
          )) {
            if (!profile) continue;
            nextProfiles[`${comboKey}:${label}`] = profile;
          }
        } catch {
          // silently skip timing for this combo
        }
      }
  ...
  async function handleRefreshTiming(game: string, state: string) {
    ...
    try {
      ...
      if (payload.success && payload.data) {
        const mergedProfiles: Record<string, TimingProfileRecord> = {};
        for (const [label, profile] of Object.entries(
          payload.data as Record<string, TimingProfileRecord>
        )) {
          if (profile) {
            mergedProfiles[`${comboKey}:${label}`] = profile;
          }
        }
        setTimingProfiles((current) => ({
          ...current,
          ...mergedProfiles,
        }));
      }
    ...
    <main className="min-h-screen bg-[#050505] text-white">
      <DashboardContainer>
        ...
        <div className="mb-6 flex rounded-full border border-[#ffbd39]/25 bg-[linear-gradient(145deg,rgba(35,19,12,0.74),rgba(10,8,8,0.92))] shadow-[0_0_18px_rgba(255,184,28,0.08)]">
          ...
          <SummaryMetric
            label="Latest Game"
            value={filteredPredictions.length > 0 ? primaryGame : '--'}
          />
        </div>
        ...
        {loading ? (
          ...
          <div className="rounded-[30px] border border-[#ffbd39]/22 bg-[linear-gradient(145deg,rgba(32,19,13,0.82),rgba(13,10,10,0.96))] px-6 py-8 text-center shadow-[0_0_22px_rgba(255,184,28,0.08)]">
            <div className="text-[36px] font-semibold tracking-[-0.03em] text-[#f6d29f]">
              No picks yet
            </div>
            <div className="mx-auto mt-3 max-w-[280px] text-[17px] leading-8 text-white/68">
              You haven&apos;t saved any picks for this filter set yet. Save a
              Strategy Locker result first if you want it to appear here and be
              eligible for play confirmation.
            </div>
            <Link
              href={
          ...
          <div className="space-y-6">
            {groupedPredictions.map((group) => (
              <section
                key={group.createdAt || 'unknown-date'}
                className="space-y-4"
              >
                ...
                <div className="space-y-4">
                  {group.picks.map((prediction) => {
                    ...
                    const timingRefreshRemaining = timingRefreshCooldowns[
                      timingComboKey
                    ]
                      ? TIMING_COOLDOWN_MS -
                        (Date.now() - timingRefreshCooldowns[timingComboKey])
                      : 0;
                    ...
                    return (
                      <PickCard
                        ...
                        isConfirmed={confirmedPredictionIds.includes(
                          prediction.id
                        )}
                        ...
                        refreshingTiming={refreshingTiming === timingComboKey}
                        cooldownRemaining={(() => {
                          return timingRefreshRemaining > 0
                            ? timingRefreshRemaining
                            : 0;
                        })()}
                      />
                    );
