...
function runSettlement(res) {
  console.log('💰 Running settlement sweep...');

  const proc = spawn('node', ['scripts/settlementSweep.js'], {
    env: process.env,
    stdio: 'pipe'
  });
  ...
  proc.on('close', (code) => {
    console.log(`💰 Settlement completed with code ${code}`);
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({
      status: code === 0 ? 'completed' : 'failed',
      exitCode: code,
  ...
  proc.on('error', (err) => {
    console.error('❌ Failed to start settlement:', err);
    res.writeHead(500, { 'Content-Type': 'application/json' });
