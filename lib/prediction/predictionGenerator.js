...
import { getPlayStyleHint } from '../brewwu/getPlayStyleHint';
...
async function getGameId(state, gameKey) {
  ...
  if (error) {
    console.error(`Game lookup error: ${state}/${gameKey} — ${error.message}`);
    return null;
  }
  ...
  if (data.length > 1 && state === 'MULTI') {
    const ncGame = data.find(g => g.state_code === 'NC');
    if (ncGame) return ncGame;
  }
 ...
 */
async function getRecentDraws(gameId, lookback = 100, drawWindow = null) {
  let query = supabase
    .from('official_draws')
    .select('*')
    .eq('game_id', gameId);
  ...
  if (drawWindow) {
    query = query.eq('draw_window_label', drawWindow);
  }
...
export async function generatePrediction({ gameKey, state, drawTime = 'midday', userId = null }) {
  try {
    ...
    const windowFilter = ['pick3', 'pick4', 'daily3', 'daily4'].includes(gameKey) ? drawTime : null;
    const draws = await getRecentDraws(game.id, 100, windowFilter);
    if (draws.length < 10) {
    ...
    const strategyKey = gameKey === 'powerball' || gameKey === 'megamillions' ? 'hotCold' : 'hotCold';
    const predictionData = {
      game_id: game.id,
      state: state === 'MULTI' ? 'NC' : state,
      game: gameKey,
      user_id: userId,
      ...
      strategy_public_label: strategyKey,
