/**
 * Multi-State Mega Millions Adapter
 * Ingests Mega Millions historical data from CSV files
 */
...
export class MultiStateMegaMillionsAdapter {
  ...
  /**
   * Ingest Mega Millions data for both NC and CA states
   */
  async ingestAll(): Promise<Record<string, IngestionResult>> {
    const results: Record<string, IngestionResult> = {};

    // Ingest for NC
    results.nc_mega_millions = await this.ingestGameState('NC', 'mega_millions', 'mega-millions.csv', 'mega_millions_official');

    // Ingest for CA
    results.ca_mega_millions = await this.ingestGameState('CA', 'mega_millions', 'mega-millions.csv', 'mega_millions_official');

    return results;
  }

  /**
   * Ingest Mega Millions data for a specific state
   */
  async ingestGameState(state: string, game: string, csvFile: string, sourceKey: string): Promise<IngestionResult> {
    ...
    try {
      ...
      for (const row of parsedRows) {
        ...
        const gameConfigKey = `${state}_MEGA_MILLIONS`;
      ...
      // Log results
      console.log(`\n📊 ${state} MEGA MILLIONS Ingestion Summary:`);
        ...
        try {
          ...
          if (dbRecords.length > 0) {
            ...
            dbRecords.slice(0, 2).forEach((record: any, i: number) => {
              console.log(`    [${i + 1}] ${record.draw_date} ${record.draw_window_label}: ${record.primary_numbers.join('-')} MB:${record.bonus_numbers.join('-')} @ ${record.draw_datetime_local}`);
            });
  ...
  private remapColumns(row: Record<string, string | number | null>, game: string): Record<string, string | number | null> {
    ...
    // Handle Mega Millions CSV format
    if (remapped.Date) {
      // Mega Millions format: "03/13/2026"
    ...
    // Remap Mega Millions columns
    // Convert "Number 1" to "n1", "Number 2" to "n2", etc.
    for (let i = 1; i <= 5; i++) {
      const numberKey = `Number ${i}`;
      if (remapped[numberKey] !== undefined) {
        remapped[`n${i}`] = remapped[numberKey];
        delete remapped[numberKey];
      }
    }

    if (remapped['Megaball'] !== undefined) {
      remapped.bonus_number = remapped['Megaball'];
      delete remapped['Megaball'];
    }

    if (remapped['Megaplier'] !== undefined) {
      remapped.multiplier = remapped['Megaplier'];
      delete remapped['Megaplier'];
    }
  ...
  private assignDrawWindows(
    ...
    draws.forEach(draw => {
      // Mega Millions draws are on Tue/Fri at 11:00 PM ET
      // For simplicity, we'll use 'nightly' as the window label
      const windowLabel = 'nightly';
      ...
      dbRecords.push({
        ...
        draw_datetime_local: `${draw.draw_date}T23:00:00-05:00`, // 11:00 PM ET
  ...
  private getGameConfig(game: string): any {
    const configs: Record<string, any> = {
      mega_millions: {
        ...
        scheduleConfig: {
          windows: [
            { label: 'nightly', time: '23:00', days: ['Tue', 'Fri'] }
          ]
        }
      }
    };
    
    return configs[game] || configs.mega_millions;
  }
}
 ...
 */
export async function ingestMultiStateMegaMillionsData(): Promise<void> {
  console.log('🚀 Starting Multi-State Mega Millions data ingestion...');

  const adapter = new MultiStateMegaMillionsAdapter();
  ...
  if (allSuccess) {
    console.log('\n✅ All Multi-State Mega Millions data ingested successfully!');
  } else {
