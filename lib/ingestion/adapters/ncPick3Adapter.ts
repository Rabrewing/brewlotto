...
export class NCPick3Adapter {
  ...
  async ingest(): Promise<IngestionResult> {
    ...
    try {
      ...
      const sourceConfig = getSourceConfig('nc_pick3_official');
      if (!sourceConfig) {
      ...
      for (const row of parsedRows) {
        ...
        const normalized = normalizeDraw(
          remappedRow,
          'NC_PICK3', // Game config key for NC Pick 3
 ...
 */
export async function ingestNCPick3Data(): Promise<void> {
