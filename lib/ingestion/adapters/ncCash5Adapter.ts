...
export class NCCash5Adapter {
  ...
  async ingest(): Promise<IngestionResult> {
    ...
    try {
      ...
      const sourceConfig = getSourceConfig('nc_cash5_official');
      if (!sourceConfig) {
      ...
      for (const row of parsedRows) {
        ...
        const normalized = normalizeDraw(
          remappedRow,
          'NC_CASH5', // Game config key for NC Cash 5
 ...
 */
export async function ingestNCCash5Data(): Promise<void> {
