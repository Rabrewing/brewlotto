        ...
        try {
            ...
            if (normalizedDraws.length > 0) {
                try {
                    ...
                    console.log(`  ℹ️ Using game_id: ${gameId} for ${sourceConfig.state}/${sourceConfig.game}`);
                    if (!sourceId) {
                        throw new Error(`Source ID not found for source key ${sourceConfig.sourceKey}`);
                    }
                    console.log(`  ℹ️ Using source_id: ${sourceId} for ${sourceConfig.sourceKey}`);
                    ...
                    if (newRecords.length > 0) {
                        const { error: insertError, data: insertedData } = await supabase
                            ...
                            .select();
                        if (insertError) {
                            console.error(`  ❌ Insertion error: ${JSON.stringify(insertError)}`);
                            ...
                            console.log(`  📥 First record: ${JSON.stringify(insertedData?.[0])}`);
                        }
                    }
                    else {
