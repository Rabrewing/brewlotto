...
export class NCPick4Adapter {
  ...
  async ingest(): Promise<IngestionResult> {
    ...
    try {
      ...
      const sourceConfig = getSourceConfig('nc_pick4_official');
      if (!sourceConfig) {
      ...
      for (const row of parsedRows) {
        ...
        const normalized = normalizeDraw(
          remappedRow,
          'NC_PICK4', // Game config key for NC Pick 4
  ...
  private remapColumns(row: Record<string, string | number | null>, game: string): Record<string, string | number | null> {
    ...
    if (remapped['Winning Numbers'] && !remapped.n1) {
      ...
      if (numbers.length >= 1) remapped.n1 = numbers[0];
      if (numbers.length >= 2) remapped.n2 = numbers[1];
      if (numbers.length >= 3) remapped.n3 = numbers[2];
      if (numbers.length >= 4) remapped.n4 = numbers[3];
 ...
 */
export async function ingestNCPick4Data(): Promise<void> {
