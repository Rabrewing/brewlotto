...
/**
 * NC Mega Millions specific parser
 */
class NCMegaMillionsParser extends BaseParser {
  constructor() {
    super({
      stateCode: 'NC',
      gameKey: 'nc-mega-millions',
  ...
  async fetch() {
    ...
    for (let i = 0; i < this.MONTHS_BACK; i++) {
      ...
      const url = `https://nclottery.com/megamillions-past-draws?month=${dt.getMonth() + 1}&year=${dt.getFullYear()}`;
      ...
      try {
        ...
        $('.past-draws-table tbody tr').each((_, el) => {
          ...
          const draw_type = $(tds[1]).text().toLowerCase().includes('tue') || $(tds[1]).text().toLowerCase().includes('fri') ? 
                            ($(tds[1]).text().toLowerCase().includes('tue') ? 'tue' : 'fri') : 'unknown';
          ...
          // Parse bonus number (Mega Ball)
          const bonusNumber = parseInt(bonusText.trim());
          const isBonusValid = !isNaN(bonusNumber) && bonusNumber >= 1 && bonusNumber <= 25;

          if (date && primaryNumbers.length === 5 && isBonusValid) {
            allDraws.push({
              draw_date: formatDate(date),
              draw_type, // tue or fri
  ...
  validate(draw) {
    return draw &&
      ...
      draw.primary_numbers.every(n => n >= 1 && n <= 70) &&
      typeof draw.bonus_number === 'number' &&
      draw.bonus_number >= 1 &&
      draw.bonus_number <= 25;
  }

  normalizeDraw(rawDraw) {
    // Determine draw window label based on day
    const drawTime = rawDraw.draw_type === 'tue' ? '20:00' : '20:00'; // Mega Millions draws at 8:00 PM ET
    return {
      game_id: null, // Will be set during ingestion process
      draw_date: rawDraw.draw_date,
      draw_window_label: rawDraw.draw_type, // tue or fri
...
module.exports = NCMegaMillionsParser;