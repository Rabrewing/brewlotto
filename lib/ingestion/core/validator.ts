/**
 * Validator - validates normalized draw data before insertion
 */
import type { NormalizedDraw } from './normalizer.js';
...
const GAME_SPECS: Record<string, { min: number; max: number; count: number; hasBonus?: boolean; bonusMin?: number; bonusMax?: number }> = {
...
export function validateDraw(draw: NormalizedDraw): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];

  // Required fields
  if (!draw.state) errors.push('Missing state');
  if (!draw.game) errors.push('Missing game');
  if (!draw.draw_date) errors.push('Missing draw_date');
  if (!draw.numbers || draw.numbers.length === 0) errors.push('Missing numbers');
  if (!draw.source_name) errors.push('Missing source_name');
  if (!draw.checksum) errors.push('Missing checksum');
...
export function validateBatch(draws: NormalizedDraw[]): { valid: number; invalid: number; results: Map<string, ValidationResult> } {
