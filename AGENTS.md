...
| **Strategy performance ratios** | `lib/stats/strategyPerformance.ts`, `app/strategy-locker/page.tsx`, `app/stats/page.tsx`, `brewdocs/v1/current_state.md`, `brewdocs/v1/CHANGELOG.md` — 2026-05-10 | Confirmed-play history now drives compact hit / win ratio chips in Strategy Locker and the Stats & Performance strategy summary so the user can see which strategies are actually working from canonical play logs rather than loose prediction closeness. |
...
| **BrewU quick index** | `app/learn/page.tsx`, `brewdocs/v1/current_state.md`, `brewdocs/v1/CHANGELOG.md` — 2026-05-12 | BrewU now has a top-of-page quick index / FAQ-style jump surface so users can jump straight to tutorial, play-style guides, Fireball, payout ladders, freshness, AI guidance, prize tables, and system links without scrolling the whole page. |
| **BrewU index polish** | `app/learn/page.tsx`, `brewdocs/v1/current_state.md`, `brewdocs/v1/CHANGELOG.md` — 2026-05-12 | The BrewU quick index now includes subtle chips and a popular-topics strip so the help hub feels more scannable on desktop and mobile without changing the page structure. |
...
| **QA Test Lab** | `app/qa/page.tsx`, `app/api/qa/submit/route.ts`, `app/api/admin/qa-reports/route.ts`, `app/api/admin/qa-reports/[id]/route.ts`, `supabase/migrations/20260515150000_add_qa_reports.sql`, `supabase/migrations/20260515150100_add_qa_screenshots_bucket.sql`, `components/brewlotto/dashboard/AvatarDropdown.tsx`, `app/login/page.tsx`, `app/auth/callback/route.ts`, `lib/auth/brewcommandShared.ts` — 2026-05-15 | Approved QA tester accounts now sign in separately from superadmins, see a gated intro overlay before the form unlocks, land in a dedicated Test Lab, and submit structured tier-by-tier reports with yes/no checks, notes, screenshots, tester identity capture, saved drafts for stop/start resume, seeded tester allowlist entries including `rb.brewington@gmail.com` in the shared auth helper, a Strategy Locker → My Picks confirmation flow, and Stripe test-card notes into a BrewCommand QA queue. |
| **Supabase security hygiene** | `supabase/migrations/20260516061446_harden_supabase_search_path_and_rls.sql` — 2026-05-16 | Hardened the helper-function `search_path` defaults and narrowed legacy draw / `pick_history` write policies to `service_role` only. This removes the mutable search-path and permissive-write lint warnings while keeping public read surfaces intact. |
| **GraphQL / RPC lock-down** | `supabase/migrations/20260516074000_lock_down_graphql_and_rpc.sql` — 2026-05-16 | Revoked anonymous schema discovery across `public`, removed authenticated access from admin/internal tables and helper views, and restricted the SECURITY DEFINER RPCs to the service role so the remaining GraphQL visibility is limited to user-owned tables the app reads directly. |
| **RLS policy cleanup** | `supabase/migrations/20260516074000_lock_down_graphql_and_rpc.sql` — 2026-05-16 | Added explicit `service_role` policies to the RLS-enabled admin/legacy tables Supabase flagged as policyless, so those service-managed tables declare their intent instead of relying on an empty policy set. |
...
| **AI provider routing** | `lib/ai/client.ts`, `app/api/brew-ai/route.ts`, `app/api/suggest-fix/route.ts`, `lib/ai/usage.ts` — 2026-05-12 | AI commentary / fix-suggestion paths are connected to OpenAI, DeepSeek, or NIM via env-driven provider selection, with token/cost logging feeding BrewCommand. The selector now accepts explicit `AI_PROVIDER=nim` as well as auto fallback. |
...
| **Superadmin added** | `.env`, `.env.local` — 2026-05-02 | `BREWCOMMAND_ADMIN_EMAILS` now includes `command@brewlotto.app`, `michael.brewington@gmail.com`, and the existing allowlist fallback so BrewCommand access works if one env entry is missing. |
...
| **Tier hook compatibility** | `hooks/useUserTier.js` — 2026-05-15 | Shared tier hook now returns both `currentTier` and legacy `tier` so older Master-gated prediction components keep reading the paid tier correctly after the entitlement-backed billing refactor. |
| **Tier source alignment** | `hooks/useUserTier.js` — 2026-05-16 | Current tier display now comes from `user_entitlements` only, removing the stale auth-metadata fallback that let Pricing and Strategy Locker disagree about whether a user was actually on Master. |
| **Billing load fallback** | `app/billing/page.tsx` — 2026-05-15 | Billing now treats the page shell as the hard dependency and degrades entitlements / tiers / features gracefully if any lookup table fails, so a table permission issue no longer collapses the whole page into `Failed to load billing`. |
| **Billing access refresh** | `app/api/billing/sync/route.ts`, `app/billing/page.tsx` — 2026-05-15 | Added a manual `Refresh Access` recovery path that rehydrates `user_entitlements` from the latest Stripe subscription record so a successful checkout can recover Master access if the webhook or entitlement write lagged. |
| **Lottery game FK diag view** | `supabase/migrations/20260516061100_fix_lottery_game_fk_diag_view.sql` — 2026-05-16 | Flipped the `public.__lottery_game_fk_diag` helper view to `security_invoker` so it no longer bypasses caller RLS and the Supabase security_definer_view lint can clear. |
| **QA tester middleware access** | `middleware.ts` — 2026-05-16 | Global auth gate now allows approved BrewCommand tester accounts as well as admins so `/qa` and the normal tester flow no longer bounce back to login before the callback redirect can land. |
| **My Picks TimePulse** | `app/my-picks/page.tsx`, `app/api/stats/timing/route.ts` — 2026-05-14 | Added TimePulse timing window chip to saved prediction cards with 36-hour refresh cooldown and blue pulsing refresh button. Integrated across Stats, Pricing, Billing, and Strategy Locker feature listings. |
...
- Customer notifications still need the canonical play-history and payout-ladder flow wired so winnings / settlement events can create in-app notifications and customer emails with return links.
