...
| **AI provider routing** | `lib/ai/client.ts`, `app/api/brew-ai/route.ts`, `app/api/suggest-fix/route.ts`, `lib/ai/usage.ts` — 2026-05-12 | AI commentary / fix-suggestion paths are connected to OpenAI, DeepSeek, or NIM via env-driven provider selection, with token/cost logging feeding BrewCommand. |
