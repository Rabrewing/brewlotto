...
| **Billing optional data fallback** | `app/billing/page.tsx` — 2026-05-15 | Billing now treats `user_entitlements` as the hard dependency and degrades tiers/features gracefully if optional lookup tables fail, so a single table permission issue no longer collapses the whole page into `Failed to load billing`. |
