...
6. **Results History / Win Ratios** — Add a confirmed-play workflow, expand result history to 3–6 months, surface win ratios by strategy/game/state in `/stats` and BrewCommand, keep `/results` grouped by draw date/time so each day’s draws read as a dated history instead of a flat list, and continue feeding the same confirmed-play signal into the Strategy Locker ratio chips and stats summaries.
